...
def get_annotate_genome(genome, bampath, learn_chrom):
    ...
    cigar_codes_rev = {0:'M', 1:'I', 2:'D', 3:'N', 4:'S', 5:'H', 6:'P'}
    ...
    #print(samfile.getrname(read.rname), file=sys.stderr)
	    ...
	    for code, length in read.cigar:
		    ...
		    cigarStrOverRead += str(cigar_codes_rev[code]) 
	    #print (read.cigar, cigarStrOverRead)
            for code, length in read.cigar:
                if code is cigar_codes['S']:
                    current_pos_read += length
                #el
		elif code is cigar_codes['M']:# or code is cigar_codes['S']:
                    if read.is_reverse:
                        #print ("read reversed", read.seq)
                        for i in xrange(length):
                            pos = ref_pos + bias + current_pos_ref + i
                            cigar_part = cigarStrOverRead[i + current_pos_read + bias : i + current_pos_read + bias + NN]
			    qgram = read.seq[i + current_pos_read + bias : i + current_pos_read + bias + NN]
			    if len(qgram) != NN or cigar_part.count('H') > 0 or cigar_part.count('D') > 0 or qgram.count('N') > 0:
			#	print ("skipped reversed:", pos, qgram, cigar_part, i, i + current_pos_read + bias - NN + 1)
                                continue
                            first_p_phred = ord(read.qual[i + current_pos_read + bias]) - 33
			    #if pos not in column:
                            #    column[pos] = []
                            #column[pos] += [(reverse_complement(qgram), cigar_part[::-1], first_p_phred, "r")]
			    if genome[pos] == 'N':
                                continue
                            if read.seq[i + current_pos_read + bias].upper() == genome[pos].upper():
                                fm[pos] += 1
				dict_fm[(reverse_complement(qgram), first_p_phred)] += 1
                            #    print ("fm", qgram, pos, genome[pos-5:pos+5])
                            else:
                                fmm[pos] += 1
				dict_fmm[(reverse_complement(qgram), first_p_phred)] += 1
                            #    print ("fmm", qgram, pos, genome[pos-5:pos+5])
                    else:
                        #print ("read direct", read.seq)
                        for i in xrange(length):
                            pos = ref_pos + bias + current_pos_ref + i
                            cigar_part = cigarStrOverRead[i + current_pos_read + bias - NN + 1 : i + current_pos_read + bias + 1]
			    qgram = read.seq[i + current_pos_read + bias - NN + 1 : i + current_pos_read + bias + 1]
			    if len(qgram) != NN or cigar_part.count('H') > 0 or cigar_part.count('D') > 0 or qgram.count('N') > 0:
			#	print ("skipped forward", pos, qgram, cigar_part, i, i + current_pos_read + bias - NN + 1)
                                continue
			    last_p_phred = ord(read.qual[i + current_pos_read + bias]) - 33
			    #if pos not in column:
                            #    column[pos] = []
                            #column[pos] += [(qgram, cigar_part, last_p_phred, "f")]
			    if genome[pos] == 'N':
                                continue
                            if read.seq[i + current_pos_read + bias].upper() == genome[pos].upper():
                                fm[pos] += 1
				dict_fm[(qgram, last_p_phred)] += 1
                            #    print ("fm", qgram, pos, genome[pos], genome[pos-5:pos+5])
                            else:
                                fmm[pos] += 1
				dict_fmm[(qgram, last_p_phred)] += 1
                            #    print ("fmm", qgram, pos, genome[pos], genome[pos-5:pos+5])
                    bias += length
    ...
    return (dict_fm, dict_fmm)#, column)
...
if __name__ == '__main__':
    ...
    fm, fmm = get_annotate_genome(genome, bampath, options.learn_chrom)
    nucls = [('A', 'C', 'G', 'T')]
    sets = []
    #for x in data:
    #    print (x)
	...
	#for pos in columns:
	 #   print("position:", pos, file=f)
	  #  for qg, cigar, phred, rev in columns[pos]:
	#	print(str(qg) + ", " + str(cigar) + ", "  + str(phred) + ", " + str(rev), file=f)
...
#                       )
    ...
    with open("report" + str(NN) + ".csv", "w") as f:
